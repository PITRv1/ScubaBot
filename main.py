...
# BOOT
boot_background = CTkLabel(app, text="", image=LoadImage("./images/blackscreen.png", default_width, default_height))
...
# ASZTAL
desktop_background = CTkLabel(app, text="", image=LoadImage("./images/SeaOS_background.png", default_width, default_height))

taskbar = CTkFrame(app, fg_color=taskbar_color, corner_radius=0)
taskbar_logo = CTkLabel(taskbar, text="", image=LoadImage("./images/SeaOS_taskbar.png", 40, 40))
...
michael_button = CTkButton(app, text="", width=50, height=50, corner_radius=0, border_width=0, fg_color="cyan", hover_color=accentcolor, image=LoadImage("./images/michael.png", 50, 50), command=lambda: LoadMichaelApp())
micheal_text = CTkLabel(app, text="buvar.run", text_color="black", bg_color="white", font=font)

system_button = CTkButton(app, text="", width=50, height=50, corner_radius=0, border_width=0, fg_color="gray", hover_color=accentcolor, image=LoadImage("./images/system.png", 50, 50), command=lambda: LoadSystemApp())
...
def LoadDesktop():
    ...
    app.after(20*timescale, lambda: taskbar_logo.place(x=5, y=5))
...
def LoadSystemApp():
    ...
    display_image = CTkLabel(inner_frame, text="", image=LoadImage("./images/display.png", 270, 220))
...
def LoadMichaelApp():
    ...
    if AnimateFrame(michwin, app, 50, 0):
        ...
        def LoadOpenFileStage():
            ...
            file_select_button.configure(command=lambda: (SelectPath(file_title, file_delfirstrow_var), file_done_button.configure(state=NORMAL)))
        ...
        def LoadMichaelSettings():
            ...
            speedtime_done_button.configure(command=lambda: SetSpeedAndTime(speed_input.get(), speed_dropdown.get(), time_input.get(), time_dropdown.get(), LoadVideos))
            speedtime_done_button.place(relx=0.5, rely=0.6, anchor=CENTER)
